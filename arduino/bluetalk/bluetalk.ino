...
static void on_bt_scan_report(const ble_gap_evt_adv_report_t *);
...
static void on_bt_timeout(uint16_t, const ble_gap_evt_timeout_t *);
...
static void input_poll() {
  ...
  while (Serial.available()) {
    ...
    } else if (line_size >= int(sizeof(input_line)) - 1) {
      Serial.printf("*** ERR=input length=%d\n", line_size);
      line_size = 0;
    } else {
...
static void on_read_command(char *args) {
  ...
  if (!*conn_text || *end) {
    Serial.printf("*** ERR=input in=read conn=\"%s\"\n", conn_text);
    return;
  }
  ...
  if (!*attr_text || *end) {
    Serial.printf("*** ERR=input in=read attr=\"%s\"\n", attr_text);
    return;
  }
...
static void on_write_command(char *args) {
  ...
  const int conn_handle = strtoul(conn_text, &end, 10);
  if (!*conn_text || *end) {
    Serial.printf("*** ERR=input in=write conn=\"%s\"\n", conn_text);
    return;
  }
  ...
  const int attr_handle = strtoul(attr_text, &end, 10);
  if (!*attr_text || *end) {
    Serial.printf("*** ERR=input in=write attr=\"%s\"\n", attr_text);
    return;
  }
  ...
  const ble_gattc_write_params_t write_params = {
    ...
    .handle = (uint16_t) attr_handle,
    .len = (uint16_t) data_size,
