...
def _bulk_steps(
    ...
    for index, chunk in enumerate(_chunks(body, chunk_size=128)):
        ...
        chunk_packets = _chunks(_encode(chunk_body, tag=tag), chunk_size=20)
        chunk_step = ProtocolStep(list(chunk_packets))
...
def set_mode(mode) -> Iterable[ProtocolStep]:
    return [ProtocolStep([_encode(struct.pack(">B", mode), tag=6)])]


def set_speed(speed) -> Iterable[ProtocolStep]:
    return [ProtocolStep([_encode(struct.pack(">B", speed), tag=7)])]


def set_brightness(brightness) -> Iterable[ProtocolStep]:
    return [ProtocolStep([_encode(struct.pack(">B", brightness), tag=8)])]
